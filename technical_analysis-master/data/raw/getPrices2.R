...
symbols2= c("AI.PA","BN.PA","DG.PA","BNP.PA","SAF.PA")

## ======== get prices with Yahoo! ===========

prices2 <-
  quantmod::getSymbols(symbols2, src = 'yahoo', from = "2012-12-28", to = "2018-12-31", auto.assign = TRUE, warnings = FALSE) %>%
  purrr::map(~Ad(get(.))) %>% 
  purrr::reduce(merge) %>%
  `colnames<-`(symbols2)
...
prices2_monthly <- 
  to.monthly(prices2, indexAt = "lastof", OHLC = FALSE)

## ==== compute returns ==========================

asset_returns_xts <- 
  PerformanceAnalytics::Return.calculate(prices2_monthly, method = "log") %>%
  na.omit()

## ================ export data ==============

## EXPORT DATA IN CVS
write.csv(as.data.frame(prices2),
          file="D:/2017-2018/data_analysis/technical_analysis/data/raw/closing_prices2.csv",
          quote= FALSE)

write.csv(as.data.frame(prices2_monthly),
          file="D:/2017-2018/data_analysis/technical_analysis/data/raw/closing_month_2.csv",
          quote= FALSE)

## ~~ make sure to go the cvs and add 'date' as a label for first column ~~

write.csv(as.data.frame(AI.PA), file='D:/2017-2018/data_analysis/technical_analysis/data/raw/AI.PA_airliquid.csv', quote= FALSE)
...
prices2 <- 
  readr::read_csv("D:/2017-2018/data_analysis/technical_analysis/data/raw/closing_prices2.csv", 
...
hc <- highchart(type = "stock") %>% 
  ...
  hc_add_series(asset_returns_xts[, symbols2[1]], name = symbols2[1]) %>%
  hc_add_series(asset_returns_xts[, symbols2[2]], name = symbols2[2]) %>%
  hc_add_series(asset_returns_xts[, symbols2[3]], name = symbols2[3]) %>%
  hc_add_series(asset_returns_xts[, symbols2[4]], name = symbols2[4]) %>%
  hc_add_series(asset_returns_xts[, symbols2[5]], name = symbols2[5]) %>%
...
asset_returns_dplyr_byhand <- 
  ...
  remove_rownames() %>%
  gather(asset, prices2, -date) %>% 
  group_by(asset) %>% 
  mutate(returns = (log(prices2) - log(lag(prices2)))) %>% 
  select(-prices2) %>% 
...
#   tidyr::gather(asset, prices2, -date) %>% 
#   dplyr::group_by(asset) %>% 
#   dplyr::mutate(returns = (log(prices2) - log(lag(prices2)))) %>% 
#   dplyr::select(-prices2) %>% 
#   tidyr::spread(asset, returns) %>% 
#   dplyr::select(date, symbols2)
...
# 
# asset_returns_tq_builtin <- prices2 %>% 
