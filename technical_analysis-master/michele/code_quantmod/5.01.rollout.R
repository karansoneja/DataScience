...
initEq=10^5
initDate="1990-12-25" ## 	A date prior to the first close price given, 
# used to contain initial account equity and initial position

strategy.st <- "strat.full" 
portfolio.st <- "portf.full"
account.st <- "acct.full"
...
## add limit positions to avoid uncontrolled trading
tradesize <- 1000
for (sym in symbols) {
  addPosLimit(portfolio.st, sym, start(get(sym)), 
              maxpos = 300, longlevels = 2)
}
...
## 0. This exit rule will execute when the market
# environment is no longer conducive to a trade (i.e. when the SMA-50 falls below SMA-200)

  
## 1. uses signal 4: executes when the RSI has crossed above 80. 
...
# won't allow exporting strategy log
